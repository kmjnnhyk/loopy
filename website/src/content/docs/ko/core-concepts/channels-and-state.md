---
title: 채널과 상태
description: loopy에서 상태는 타입이 붙은 채널들의 레코드예요. 채널마다 리듀서가 선언되어 있어요. 워크플로우와 팀은 업데이트를 채널에 접어 넣고, 라우터는 타입이 붙은 스냅샷을 보고 갈래를 나눠요.
---

## 상태는 변수가 아니라 채널이에요

워크플로우에는 — 같은 메커니즘 위에 지어진 팀도 마찬가지로 — "변수"가 없어요. 대신 **채널**이 있어요. 채널은 값 타입, 업데이트 타입, 그리고 들어온 업데이트를 현재 값에 접어 넣는 리듀서를 가진, 이름이 붙은 슬롯이에요:
...
`V`와 `U`는 서로 다를 수 있어요 — `listChannel`의 값은 `readonly T[]`인데, 새 아이템 하나만 건네도 되고 배치로 건네도 돼요. 노드는 상태를 직접 변경하지 않아요. 업데이트를 반환할 뿐이고, 런타임이 각 업데이트를 해당 채널의 `reduce`로 접어 넣어요. 그래서 설계 전체가 엔진을 **`state = fold(reduce, log, initial)`**이라는 불변식 하나로 설명하는 거예요. 라우터가 읽는 실시간 상태는 이 fold의 캐시일 뿐이고, 그 밑에 있는 추가 전용 이벤트 로그가 유일한 진짜 권위예요. 이 불변식은 런타임이 생긴 뒤 [이벤트 소싱 리플레이](/ko/core-concepts/event-sourcing/)를 가능하게 하는 것이기도 해요 — 로그를 리플레이한다는 건 그저 같은 fold를 다시 실행하는 것뿐이에요.
...
`inputChannel`은 [`team()`](/ko/reference/team/)에서 쓰여요. 값/업데이트 레벨에서는 `lastChannel`과 형태가 같지만, *브랜드*가 붙어 있어요(`readonly "~input": true`). 이 브랜드 덕분에 타입 시스템이 팀의 일반 도메인 채널과 별개로, 실행 입력을 구성하는 채널만 정확히 골라낼 수 있어요:
...
채널들의 레코드가 주어지면, `StateOf`는 라우터가 실제로 보는 평범한 객체 형태를 알려줘요 — 채널 래퍼가 아니라 각 채널의 *값* 타입으로요:

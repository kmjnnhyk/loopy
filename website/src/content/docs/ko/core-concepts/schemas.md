...
툴의 입력과 출력에는 두 가지가 동시에 필요해요. 컴파일러가 검사하는 **정적 TypeScript 타입**과, 언젠가는 붙어야 할 **런타임 검증기**예요. 이 검증기는 LLM이 실제로 돌려주는 값(깨진 JSON, 마크다운 코드 펜스, 트레일링 콤마)을 그 타입으로 강제 변환해야 해요. loopy는 이 문제를 `IO`라는 캐리어 타입 하나로 풀어요. [Standard Schema](https://standardschema.dev/) 스펙과 같은 형태라서, 이 스펙을 구현하는 검증기 라이브러리라면 Zod든 Valibot이든 ArkType이든 무엇이든 loopy가 특정 라이브러리에 의존하지 않고도 바로 갈아 끼울 수 있어요.
...
모든 [`tool()`](/ko/reference/tool/), [`agent()`](/ko/reference/agent/), 워크플로우 노드는 `io<...>()`로 만든 `input`과 `output` 스키마를 받아요. 실제 검증기가 연결된 뒤에는, 같은 `~standard` 형태를 구현하는 Zod나 Valibot, ArkType 스키마를 대신 받을 수도 있고요.

...
가장 빠른 시작은 바로 실행되는 프로젝트를 스캐폴딩하는 거예요:

```bash
bun create loopy@next my-app
cd my-app
bun install
bun run dev            # http://localhost:5173 에 DevTools UI가 열려요
```

`@loopyjs/core`와 `loopy` CLI, DevTools가 연결되고, `dev` 스크립트와 바로 실행되는 `index.ts`(아래 `hello` 워크플로우)까지 갖춘 Bun 프로젝트가 만들어져요 — 추가 설정도, API 키도 필요 없어요.

### 직접 설치

기존 프로젝트에 loopy를 얹으려면:

```bash
bun init -y                    # 새 프로젝트라면 먼저

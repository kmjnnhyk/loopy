---
title: "가이드: 툴 만들기"
description: 툴을 단계별로 만들어요 — 의존성을 선언하고, 본문을 작성하고, 멱등성 계약을 이해해요.
---
...
툴을 작성하기 전에, 앱은 `LoopyDeps`를 확장해서 자신의 의존성을 선언해요:
...
툴의 `input`/`output`은 [`IO<...>`](/ko/core-concepts/schemas/) 스키마예요. 프로토타입 단계에서는 `io<T>()`가 항등 밸리데이터가 붙은 정적 타입을 줘요:
...
이 키를 "다시 발생해도 이게 *같은* 호출이라는 걸 알려주는 것"이라고 생각하세요 — 보통 입력 전체가 아니라, 의미상 식별에 필요한 필드들의 안정적인 해시예요.

## 결과

`editFile`은 완전히 타입이 붙은 [`Step`](/ko/core-concepts/step/)이에요. `name`은 리터럴 `"editFile"`로 보존되고, 의존성 요구사항은 정확히 `"repo"`이며, `run` 시그니처는 `(input: { path: string; find: string; replace: string }, ctx: ToolCtx<"repo">) => Promise<{ applied: boolean }>`예요. 이제 [`agent()`](/ko/reference/agent/)의 `tools` 배열에 넣거나 [`workflow()`](/ko/reference/workflow/)의 노드로 바로 쓸 수 있어요.

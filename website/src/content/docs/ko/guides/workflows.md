---
title: 결정적 워크플로우
description: 상태 채널과 라우터로, 재시도 사이클과 분기가 있는 2단계 워크플로우를 만들어요.
---
...
노드 자리에는 `tool()`, `agent()`, 인라인 `step()` 중 무엇이든 올 수 있어요. 예를 들어 `verify: verifier`는 `verifier` 에이전트를 `"verify"`라는 노드 이름으로 매핑한 거고, `build`와 `push`는 인라인 `step()`이에요(아래에서 다뤄요). 엣지를 잇기 전에 노드 전체를 먼저 선언해두면 좋은 점이 있어요. 다음 단계에서 체인상 "뒤에" 오는 노드까지 포함해, *어떤* 노드든 순서에 상관없이 참조할 수 있거든요. 전방 참조 에러는 나지 않아요.
...
재시도 루프는 `.branch("build", (s) => ...)` 안에 있어요. 빌드가 실패하면 `codeGen`으로 돌아가서 다시 시도해요. loopy는 사이클을 다른 엣지와 구분하지 않아요. 그저 우연히 앞선 노드를 가리키는 이름을 반환하는 라우터가 하나 있을 뿐이에요. 여기서 `s`는 `StateOf<typeof state>`라서, `s.build?.ok`는 실제로 좁혀지는 프로퍼티 접근이에요.

`.branch("build", (s) => (s.build?.ok ? "verfy" : "codeGen"))`처럼 오타를 내보세요. `TS2820`이 뜨고 "verify'를 의도하신 건가요?" 같은 제안까지 나와요. 라우터의 반환 타입이 노드 이름들의 정확한 리터럴 유니온이기 때문이에요.
